...
# adding much of patients i eeach study
dat2$SampleSize <- dat2$n1i + dat2$n2i

# Fit model
bmi.m1 <- rma(yi, vi, data = dat2)

# Generate a more aesthetic forest plot
forest(bmi.m1,
...
stripchart(yi ~ factor(exercise, labels = c('Without', 'With')),
           ...
           xlab = 'Exercise',
           ylab = 'Reduction in BMI',
           main = 'Estimate = 0.82, P = 0.02')

# Tambahkan garis rata-rata dan error bars
...
dat2$counseling <- as.factor(dat2$counseling)
xc <- c('Without', 'With')[dat3$counseling]
stripchart(yi ~ factor(counseling, labels = c('Without', 'With')),
           data = dat2,
           vertical = TRUE,
           method = 'jitter',
           jitter = 0.2,
           ...
           xlab = 'Counseling',
           ylab = 'Reduction in BMI',
           main = 'Estimate = -0.72, P = 0.10')

coun.ave <- aggregate(dat2$yi, by = list(counseling = dat2$counseling), mean)
coun.sd  <- aggregate(dat2$yi, by = list(counseling = dat2$counseling), sd)

points(1:2, coun.ave$x, pch = 19, col = 'red')
segments(1:2, coun.ave$x - coun.sd$x, 1:2, coun.ave$x + coun.sd$x, col = 'red', lwd = 1.5)

dat2$diet_note <- as.factor(dat2$diet_note)

stripchart(dat2$yi ~ dat2$diet_note,
           vertical = TRUE,
           method = 'jitter',
           pch = 1,
           cex = dat2$vi + 1,
           lwd = 2,
           xlab = 'Diet Description',
           ylab = 'Reduction in BMI',
           main = 'BMI Reduction per Diet Note')

# Tambahkan mean dan SD jika cocok
dietnote.ave <- aggregate(dat2$yi, by = list(note = dat2$diet_note), mean)
dietnote.sd  <- aggregate(dat2$yi, by = list(note = dat2$diet_note), sd)

points(1:nrow(dietnote.ave), dietnote.ave$x, pch = 19, col = 'red')
segments(1:nrow(dietnote.ave), dietnote.ave$x - dietnote.sd$x,
         1:nrow(dietnote.ave), dietnote.ave$x + dietnote.sd$x,
         col = 'red', lwd = 1.5)
...
stripchart(dat2$yi ~ dat2$diet,
           ...
           xlab = 'Diet Type',
           ylab = 'Reduction in BMI',
           main = 'Estimate diet reduced = 0.46, P = 0.30')
